...
class Math_Captcha_Core {
	...
	public function load_actions_filters() {
        ...
        {
            // Check IP rules
            if (isset(Math_Captcha()->options['general']['ip_rules']) && Math_Captcha()->options['general']['ip_rules'])
            {
                $geo = new MathCaptcha_GEO();
                if ($geo->checkIP_in_List(false, Math_Captcha()->options['general']['ip_rules_list'])) return; // Dont show captcha
            }
            // Check GEO rules
            if ( isset(Math_Captcha()->options['general']['geo_captcha_rules']) && Math_Captcha()->options['general']['geo_captcha_rules'])
            {
                $geo = new MathCaptcha_GEO();
